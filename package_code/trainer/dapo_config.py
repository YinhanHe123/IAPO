...
class DAPOConfig(TrainingArguments):
    ...
    This class includes only the parameters that are specific to DAPO training. For a full list of training arguments,
    ...
    Parameters:
        ...
        delta: (`float` or `None`, *optional*, defaults to `None`):
            Enables the upper clipping bound in two-sided DAPO loss when set to a float. If `None` (default), standard
            DAPO clipping is used. Recommended to be greater than `1 + ε` when enabled. This method is introduced in
        ...
        scale_rewards (`bool`, *optional*, defaults to `True`):
            ...
            applied. The [Dr. DAPO paper](https://huggingface.co/papers/2503.20783) recommends not scaling the rewards,
        ...
        loss_type (`str`, *optional*, defaults to `"bnpo"`):
            ...
            - `"dapo"`: Aggregates token-level losses by normalizing over sequence length. Not recommended due to
            ...
            - `"bnpo"`: Aggregates token-level losses by normalizing number of active token in the local batch.
                ...
                `per_device_train_batch_size==1`, the loss is equivalent to the DAPO loss.
            - `"dr_dapo"`: Aggregates token-level losses by normalizing with a global constant. This method was
                introduced in the [Dr. DAPO paper](https://huggingface.co/papers/2503.20783) to eliminate length bias.
    ...
    # Parameters that control the data preprocessing
    # The default value remove_unused_columns is overwritten from the parent class, because in DAPO we usually rely on
    ...
    delta: Optional[float] = field(
        default=None,
        metadata={
            "help": "Enables the upper clipping bound in two-sided DAPO loss when set to a float. If `None` "
            "(default), standard DAPO clipping is used. Recommended to be greater than `1 + ε` when enabled. This "
    ...
    scale_rewards: bool = field(
        ...
        metadata={
            ...
            "scaling is applied. The Dr. DAPO paper recommends not scaling the rewards, as scaling by the standard "
    ...
    loss_type: str = field(
        default="bnpo",
        metadata={
            "help": "Specifies the loss formulation to use. Supported values are `dapo`, `bnpo`, and `dr_dapo`. "
            "`'dapo'`: Aggregates token-level losses by normalizing over sequence length. Not recommended due to "
            "length bias—this approach tends to prefer shorter completions with positive advantages and longer ones "
            "with negative advantages. "
            "`'bnpo'`: Aggregates token-level losses by normalizing number of active token in the local batch. "
            ...
            "`per_device_train_batch_size==1`, the loss is equivalent to the DAPO loss. "
            "`'dr_dapo'`: Aggregates token-level losses by normalizing with a global constant. This method was "
            "introduced in the Dr. DAPO paper to eliminate length bias. The value of the constant corresponds to "
    ...
    def __post_init__(self):
        ...
        if self.delta is not None and self.use_liger_loss:
            raise ValueError("Liger loss does not support two-sided DAPO loss yet.")
