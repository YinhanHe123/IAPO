...
from .s_grpo_lee_config import SGRPOLeeConfig
from .utils import (
...
if is_liger_kernel_available():
    from liger_kernel.chunked_loss import LigerFusedLinearSGRPOLeeLoss
...
class SGRPOLeeTrainer(Trainer):
    """
    Trainer for the Group Relative Policy Optimization (SGRPOLee) method. This algorithm was initially proposed in the
    ...
    from trl import SGRPOLeeTrainer
    ...
    Args:
        ...
        args ([`SGRPOLeeConfig`], *optional*, defaults to `None`):
    ...
    _tag_names = ["trl", "s_grpo_lee"]

    def __init__(
        self,
        model: Union[str, PreTrainedModel],
        reward_funcs: Union[RewardFunc, list[RewardFunc]],
        args: Optional[SGRPOLeeConfig] = None,
        ...
        if args is None:
            ...
            args = SGRPOLeeConfig(f"{model_name}-SGRPOLee")
        self.alpha = args.alpha
        self.k = args.k
        self.token_prob = args.token_prob
        ...
        if isinstance(model, str):
            ...
            if isinstance(torch_dtype, torch.dtype) or torch_dtype == "auto" or torch_dtype is None:
                ...
                raise ValueError(
                    "Invalid `torch_dtype` passed to `SGRPOLeeConfig`. Expected either 'auto' or a string representing "
            ...
            if args.model_init_kwargs is not None:
                raise ValueError(
                    "You passed `model_init_kwargs` to the `SGRPOLeeConfig`, but your model is already instantiated. "
        ...
        self.max_completion_length = args.max_completion_length  # = |o_i| in the SGRPOLee paper
        self.num_generations = args.num_generations  # = G in the SGRPOLee paper
        ...
        if (
            ...
            raise NotImplementedError(
                "Iterable datasets are not yet supported in SGRPOLeeTrainer. Please use a standard dataset instead."
            )

        # Multi-step
        self.num_iterations = args.num_iterations  # = 𝜇 in the SGRPOLee paper
        ...
        # input tensor associated with the key "input_ids". However, in SGRPOLee, the sampled data does not include the
        ...
        super().__init__(
            model=model,
            args=args,
            data_collator=identity,  # No data collation is needed in SGRPOLee
        ...
        if self.use_liger_loss:
            if not is_liger_kernel_available():
                raise ImportError(
                    "Liger is required to use `liger_loss` as the SGRPOLee loss. Run `pip install liger-kernel`."
                )
            # redirect the model.module forward to the model forward to ensure pre-forward hooks are called
            self._forward_redirection = _ForwardRedirection()

            self.liger_s_grpo_lee_loss = LigerFusedLinearSGRPOLeeLoss(
                beta=self.beta,
    ...
    def _set_signature_columns_if_needed(self):
        ...
        # In SGRPOLeeTrainer, we preprocess data, so using the model's signature columns doesn't work.
    ...
    # modification. As a result, some parts of the method aren't relevant to SGRPOLee, but we keep them to stay one line
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel, args: SGRPOLeeConfig) -> PreTrainedModel:
    ...
    def compute_liger_loss(self, unwrapped_model, inputs):
        ...
        # compute loss and metrics using liger s_grpo_lee loss
        loss, metrics = self.liger_s_grpo_lee_loss(
        ...
        # Extract metrics from the liger_s_grpo_lee_loss output
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            raise ValueError("The SGRPOLeeTrainer does not support returning outputs")
        if self.use_liger_loss:
            # Compute the loss using the liger s_grpo_lee loss
    ...
    def _compute_loss(self, model, inputs):
        ...
        if self.loss_type == "grpo":
            tensor_width = completion_mask.shape[-1]
            completion_lengths = torch.sum(completion_mask,-1)
            if_bernulli = (completion_lengths < self.k)
            length_mask = torch.zeros_like(completion_mask)
            if self.alpha <= tensor_width:
                length_mask[:,:self.alpha] = 1 
                bernoulli_mask = torch.ones_like(length_mask[if_bernulli,self.alpha:min(tensor_width, self.k)]) * self.token_prob 
                # Synchronize random seed across ranks to avoid NCCL timeout
                generator = torch.Generator(device=completion_mask.device).manual_seed(self.state.global_step + 42)
                bernoulli_mask = torch.bernoulli(bernoulli_mask, generator=generator)
                length_mask[if_bernulli, self.alpha:min(tensor_width, self.k)] = bernoulli_mask.to(length_mask.dtype)
            else:
                length_mask = 1 
            completion_mask *= length_mask
