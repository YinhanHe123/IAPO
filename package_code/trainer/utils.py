...
def entropy_from_attn(attention: torch.Tensor) -> torch.Tensor:
    """
    Compute the Shannon entropy (in nats) from attention weights.

    Args:
        attention (`torch.Tensor`):
            Attention weights tensor of shape `(B, num_heads, query_length, key_length)`. We take entropy along the last row of each attention matrix and finally mean over the heads.
    Returns:
        `torch.Tensor`:
            Entropy values with shape `(B, query_length)`.
    """
    prompt_prefix = 50
    all_tokens_entropy = []
    for i in range(attention.shape[-2]):
        att_token_i = attention[:,:,i,:][:,:,:i+prompt_prefix]  # (B, num_heads, key_length)
        att_token_i = F.softmax(att_token_i, dim=-1)
        # entropy_token_i = -(att_token_i * (att_token_i + 1e-8).log()).mean(-1).mean(-1)/(torch.log(torch.tensor(i+prompt_prefix))+1e-8)  # (B), the last term is normalization for the length
        entropy_token_i = -(att_token_i * (att_token_i + 1e-8).log()).sum(-1).mean(-1)/(torch.log(torch.tensor(i+prompt_prefix))+1e-8)  # (B), the last term is normalization for the length
        all_tokens_entropy.append(entropy_token_i)
    attn_entropy = torch.stack(all_tokens_entropy, dim=-1)  # (B, query_length)
    # --------------------temp code for drawing -----------------------
    # attn_entropy_to_plot = attn_entropy[0].unsqueeze(0)
    # data_np = attn_entropy_to_plot.cpu().detach().numpy()
    ...
    #     plt.savefig(os.path.join(output_dir, f'{entropy_from_attn.x}.png'), dpi=150)
    #     plt.close()
    #     entropy_from_attn.x += 1
     # --------------------end temp code for drawing -----------------------
    return attn_entropy

# entropy_from_attn.x = 0
