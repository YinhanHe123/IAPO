...
from .gtpo_config import GTPOConfig
from .utils import (
...
if is_liger_kernel_available():
    from liger_kernel.chunked_loss import LigerFusedLinearGTPOLoss
...
class GTPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if args is None:
            ...
            args = GTPOConfig(f"{model_name}-GTPO")
        ...
        if isinstance(model, str):
            ...
            if isinstance(torch_dtype, torch.dtype) or torch_dtype == "auto" or torch_dtype is None:
                ...
                raise ValueError(
                    "Invalid `torch_dtype` passed to `GTPOConfig`. Expected either 'auto' or a string representing "
            ...
            if args.model_init_kwargs is not None:
                raise ValueError(
                    "You passed `model_init_kwargs` to the `GTPOConfig`, but your model is already instantiated. "
        ...
        if (
            ...
            raise NotImplementedError(
                "Iterable datasets are not yet supported in GTPOTrainer. Please use a standard dataset instead."
            )
 
        # Multi-step
        self.num_iterations = args.num_iterations  # = 𝜇 in the GTPO paper
        ...
        # input tensor associated with the key "input_ids". However, in GTPO, the sampled data does not include the
        ...
        super().__init__(
            model=model,
            args=args,
            data_collator=identity,  # No data collation is needed in GTPO
        ...
        if self.use_liger_loss:
            if not is_liger_kernel_available():
                raise ImportError(
                    "Liger is required to use `liger_loss` as the GTPO loss. Run `pip install liger-kernel`."
                )
            # redirect the model.module forward to the model forward to ensure pre-forward hooks are called
            self._forward_redirection = _ForwardRedirection()
 
            self.liger_gtpo_loss = LigerFusedLinearGTPOLoss(
                beta=self.beta,
    ...
    def _set_signature_columns_if_needed(self):
        ...
        # In GTPOTrainer, we preprocess data, so using the model's signature columns doesn't work.
    ...
    # modification. As a result, some parts of the method aren't relevant to GTPO, but we keep them to stay one line
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel, args: GTPOConfig) -> PreTrainedModel:
    ...
    def _generate_and_score_completions(
        ...
        with torch.no_grad():
            ...
            _, per_token_entropies = self._get_per_token_logps_and_entropies(self.model,
            ...
            # Gather per_token_entropies across all processes for group-wise operations
            completion_lengths_gathered = self.accelerator.gather(completion_lengths)
            max_seq_len = completion_lengths_gathered.max().item()
 
            # Pad sequence dimension with 0.0 if needed
            if per_token_entropies.shape[1] < max_seq_len:
                pad_width = max_seq_len - per_token_entropies.shape[1]
                per_token_entropies = torch.nn.functional.pad(per_token_entropies, (0, pad_width), value=0.0)
 
            # Pad batch dimension (dim=0) across processes
            padded_entropies = self.accelerator.pad_across_processes(per_token_entropies, dim=0, pad_index=0.0)
 
            # Gather across all processes
            per_token_entropies = self.accelerator.gather(padded_entropies)
        ...
        # when normalizing, see if the sequence is positive reward or negative reward.
        # if positive, normalize with H_{i,t}/\Sigma_{k=1}^{n}{H_{k,t}}*d_t, i.e., the tokens within the group in the same position (i.e., column). For short sequences that the token position is dummy token (eos), assign 0 to entropy at that term
        rewards_grouped = rewards.view(-1, self.num_generations)
        per_token_entropies_grouped = per_token_entropies.view(-1, self.num_generations, per_token_entropies.size(-1))
 
        # Gather completion_mask and pad to match per_token_entropies dimensions
        completion_mask_padded = completion_mask
        if completion_mask_padded.shape[1] < per_token_entropies.size(-1):
            pad_width = per_token_entropies.size(-1) - completion_mask_padded.shape[1]
            completion_mask_padded = torch.nn.functional.pad(completion_mask_padded, (0, pad_width), value=0)
        completion_mask_padded = self.accelerator.pad_across_processes(completion_mask_padded, dim=0, pad_index=0)
        completion_mask_gathered = self.accelerator.gather(completion_mask_padded)
 
        completion_mask_grouped = completion_mask_gathered.view(-1, self.num_generations, completion_mask_gathered.size(-1))
        advantages_grouped = advantages.view(-1, self.num_generations).unsqueeze(-1)
        for i in range(per_token_entropies_grouped.size(0)):
            positive_idxs = rewards_grouped[i] >= 0
            negative_idxs = rewards_grouped[i] < 0
            # Fix: sum across sequences (dim=0) at each position, not across positions
            if positive_idxs.any():
                entropy_sum = per_token_entropies_grouped[i][positive_idxs].sum(0, keepdim=True) + 1e-8
                seq_lengths = completion_mask_grouped[i][positive_idxs].sum(1, keepdim=True)  # (num_pos, 1)
                per_token_entropies_grouped[i][positive_idxs] = 0.1 * per_token_entropies_grouped[i][positive_idxs] / entropy_sum * seq_lengths
            if negative_idxs.any():
                # Per Equation (3): use inverse entropy to penalize confident mistakes more heavily
                # Low entropy (high confidence) → large 1/H → more penalty
                # High entropy (uncertain) → small 1/H → less penalty (encourages exploration)
                inv_entropy = 1.0 / (per_token_entropies_grouped[i][negative_idxs] + 1e-8)
                inv_entropy_sum = inv_entropy.sum(0, keepdim=True) + 1e-8
                seq_lengths = completion_mask_grouped[i][negative_idxs].sum(1, keepdim=True)  # (num_neg, 1)
                per_token_entropies_grouped[i][negative_idxs] = -0.1 * inv_entropy / inv_entropy_sum * seq_lengths

        # Per equation (2): token advantage = group advantage + entropy-based token reward
        advantages_grouped = advantages_grouped + per_token_entropies_grouped
        # Slice to keep only the local part of the data
        advantages = advantages_grouped.view(-1, advantages_grouped.size(-1))
        process_slice = slice(
            self.accelerator.process_index * len(prompts),
            (self.accelerator.process_index + 1) * len(prompts),
        )
    ...
    def compute_liger_loss(self, unwrapped_model, inputs):
        ...
        # compute loss and metrics using liger gtpo loss
        loss, metrics = self.liger_gtpo_loss(
        ...
        # Extract metrics from the liger_gtpo_loss output
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            raise ValueError("The GTPOTrainer does not support returning outputs")
        if self.use_liger_loss:
            # Compute the loss using the liger gtpo loss
