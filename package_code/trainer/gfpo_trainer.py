...
if is_liger_kernel_available():
    from liger_kernel.chunked_loss import LigerFusedLinearGFPOLoss
...
class GFPOTrainer(Trainer):
    """
    Trainer for the Group Relative Policy Optimization (GFPO) method. This algorithm was initially proposed in the
    ...
    Args:
        ...
        args ([`GFPOConfig`], *optional*, defaults to `None`):
    ...
    def __init__(
        ...
        if args is None:
            ...
            args = GFPOConfig(f"{model_name}-GFPO")
        ...
        if isinstance(model, str):
            ...
            if isinstance(torch_dtype, torch.dtype) or torch_dtype == "auto" or torch_dtype is None:
                ...
                raise ValueError(
                    "Invalid `torch_dtype` passed to `GFPOConfig`. Expected either 'auto' or a string representing "
            ...
            if args.model_init_kwargs is not None:
                raise ValueError(
                    "You passed `model_init_kwargs` to the `GFPOConfig`, but your model is already instantiated. "
        ...
        self.max_completion_length = args.max_completion_length  # = |o_i| in the GFPO paper
        self.num_generations = args.num_generations  # = G in the GFPO paper
        ...
        if (
            ...
            raise NotImplementedError(
                "Iterable datasets are not yet supported in GFPOTrainer. Please use a standard dataset instead."
            )

        # Multi-step
        self.num_iterations = args.num_iterations  # = 𝜇 in the GFPO paper
        ...
        # input tensor associated with the key "input_ids". However, in GFPO, the sampled data does not include the
        ...
        super().__init__(
            model=model,
            args=args,
            data_collator=identity,  # No data collation is needed in GFPO
        ...
        if self.use_liger_loss:
            if not is_liger_kernel_available():
                raise ImportError(
                    "Liger is required to use `liger_loss` as the GFPO loss. Run `pip install liger-kernel`."
                )
            # redirect the model.module forward to the model forward to ensure pre-forward hooks are called
            self._forward_redirection = _ForwardRedirection()

            self.liger_gfpo_loss = LigerFusedLinearGFPOLoss(
                beta=self.beta,
    ...
    def _set_signature_columns_if_needed(self):
        ...
        # In GFPOTrainer, we preprocess data, so using the model's signature columns doesn't work.
    ...
    # modification. As a result, some parts of the method aren't relevant to GFPO, but we keep them to stay one line
    ...
    def _enable_gradient_checkpointing(self, model: PreTrainedModel, args: GFPOConfig) -> PreTrainedModel:
    ...
    def _get_per_token_logps_and_entropies(
        ...
        # print(f"[GFPO Debug] Line 798: input_ids.size(0): {input_ids.size(0)}")
        if input_ids.size(0) == 0:
            device = input_ids.device
            empty_logps = torch.zeros((0, logits_to_keep), dtype=torch.float32, device=device)
            empty_entropies = torch.zeros((0, logits_to_keep), dtype=torch.float32, device=device) if compute_entropy else None
            return empty_logps, empty_entropies
    ...
    def _generate_and_score_completions(
        ...
        # # Select top shortest completions per group if top_num_gen is set
        # # Only apply filtering during training, not during evaluation
        # original_num_generations = self.num_generations  # Store original value
        original_num_generations = self.num_generations
        if mode == "train" and hasattr(self.args, 'top_num_gen') and self.args.top_num_gen is not None:
            top_k = self.args.top_num_gen
            
            # Reshape to group structure: (num_prompts, num_generations)
            # print(f"[GFPO Debug] Line 1370:completion_lengths (should be 8): {len(prompts)}")
            # print(f"[GFPO Debug] Line 1371: num_generations: {self.num_generations}")
            num_prompts = completion_lengths.size(0) // self.num_generations
            
            # Check if this rank has insufficient or partial data to perform filtering
            # This happens when completion_lengths.size(0) < num_generations or is not evenly divisible
            # due to uneven data distribution across ranks or the last batch having fewer samples
            batch_size = completion_lengths.size(0)
            is_partial_batch = batch_size % self.num_generations != 0
            
            if num_prompts == 0 or is_partial_batch: # original version: if numprimpts == 0:
                # Skip filtering on this rank to avoid empty batch or shape mismatch
                # Log a warning so users can adjust batch sizes to avoid this
                if self.accelerator.is_main_process:
                    warnings.warn(
                        f"Rank {self.accelerator.process_index} has insufficient/partial data for top_num_gen filtering "
                        f"(received {batch_size} samples, need multiple of {self.num_generations}). "
                        f"Skipping filtering on this rank. Consider increasing per_device_train_batch_size or "
                        f"decreasing world_size to ensure each rank has samples divisible by {self.num_generations}.",
                        UserWarning
                    )
            else:
                completion_lengths_grouped = completion_lengths.view(num_prompts, self.num_generations)
                
                # Get indices of top k shortest within each group
                _, indices = torch.topk(completion_lengths_grouped, k=min(top_k, self.num_generations), 
                                        dim=1, largest=False, sorted=True)
                
                # Create flat indices for selecting from the batch
                batch_indices = torch.arange(num_prompts, device=device).unsqueeze(1).expand(-1, top_k)
                flat_indices = batch_indices * self.num_generations + indices
                flat_indices = flat_indices.flatten()
                
                # Filter all relevant tensors
                prompt_ids = prompt_ids[flat_indices]
                prompt_mask = prompt_mask[flat_indices]
                completion_ids = completion_ids[flat_indices]
                completion_mask = completion_mask[flat_indices]
                prompt_completion_ids = prompt_completion_ids[flat_indices]
                is_eos = is_eos[flat_indices]
                completion_lengths = completion_lengths[flat_indices]
                prompts_text = [prompts_text[i] for i in flat_indices.cpu().tolist()]
                # completions_text = [completions_text[i] for i in flat_indices.cpu().tolist()]
                # completions = [completions[i] for i in flat_indices.cpu().tolist()]
                completion_ids_list = [completion_ids_list[i] for i in flat_indices.cpu().tolist()]
                original_prompts = [original_prompts[i] for i in flat_indices.cpu().tolist()]
                prompts = [prompts[i] for i in flat_indices.cpu().tolist()]
                inputs = [inputs[i] for i in flat_indices.cpu().tolist()]
                
                if has_images:
                    images = [images[i] for i in flat_indices.cpu().tolist()]
                
                # Filter logprobs if they exist
                # if self.use_vllm:
                #     sampling_per_token_logps = sampling_per_token_logps[flat_indices]
                
                # Update num_generations for subsequent processing
                self.num_generations = top_k
                
                # Recompute aggregated lengths after filtering
                agg_completion_lengths = self.accelerator.gather(completion_lengths)
        ...
        # Restore original num_generations
    ...
    def compute_liger_loss(self, unwrapped_model, inputs):
        ...
        # Safety check: handle edge case where batch might be empty
        # This should rarely happen now due to prevention logic in filtering, but kept as failsafe
        if prompt_ids.size(0) == 0:
            warnings.warn(
                "Empty batch detected in compute_liger_loss. This should be rare with the updated filtering logic.",
                UserWarning
            )
            return unwrapped_model.lm_head.weight.sum() * 0.0
        ...
        # compute loss and metrics using liger gfpo loss
        loss, metrics = self.liger_gfpo_loss(
        ...
        # Extract metrics from the liger_gfpo_loss output
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        if return_outputs:
            raise ValueError("The GFPOTrainer does not support returning outputs")
        if self.use_liger_loss:
            # Compute the loss using the liger gfpo loss
    ...
    def _compute_loss(self, model, inputs):
        ...
        if prompt_ids.size(0) == 0:
            warnings.warn(
                "Empty batch detected in _compute_loss. This should be rare with the updated filtering logic.",
                UserWarning
            )
            unwrapped_model = self.accelerator.unwrap_model(model)
            return unwrapped_model.lm_head.weight.sum() * 0.0
