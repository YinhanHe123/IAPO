...
if is_liger_kernel_available():
    from liger_kernel.chunked_loss import LigerFusedLinearDAPOLoss
...
class DAPOTrainer(Trainer):
    """
    Trainer for the Group Relative Policy Optimization (DAPO) method. This algorithm was initially proposed in the
    ...
    def __init__(
        ...
        self.max_completion_length = args.max_completion_length  # = |o_i| in the DAPO paper
        self.num_generations = args.num_generations  # = G in the DAPO paper
        ...
        # Multi-step
        self.num_iterations = args.num_iterations  # = 𝜇 in the DAPO paper
        ...
        # input tensor associated with the key "input_ids". However, in DAPO, the sampled data does not include the
        ...
        super().__init__(
            model=model,
            args=args,
            data_collator=identity,  # No data collation is needed in DAPO
        ...
        if self.use_liger_loss:
            ...
            self.liger_dapo_loss = LigerFusedLinearDAPOLoss(
                beta=self.beta,
    ...
    # modification. As a result, some parts of the method aren't relevant to DAPO, but we keep them to stay one line
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.use_liger_loss:
            # Compute the loss using the liger dapo loss
